...
function MyPage() {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 401) {
        try {
          const refreshRes = await axios.post("http://localhost:5000/refresh", {}, { withCredentials: true });
  ...
  return (
    <div className="container pullDown">
      <div>
        <h1>마이페이지</h1>
        <div className="MyPageDescription">-나의 정보를 확인하고 관리해요! -</div>
        <a href="#info" onClick={() => handleLinkClick("내 정보")}>내 정보</a>
        <a href="#stamp" onClick={() => handleLinkClick("도장판")}>도장판</a>
        <a href="#learn" onClick={() => handleLinkClick("학습 진행률")}>학습 진행률</a>
        <a href="#album" onClick={() => handleLinkClick("오답 앨범")}>오답 앨범</a>
      </div>
      <div className="divider"></div>
      {selectedContent && <div className="content">{getContentComponent()}</div>}
    </div>
  );
}
...
function UserInfo({ userData, navigate }) {
  ...
  const handleGenderChange = async (selectedGender) => {
    ...
    try {
      await axios.post("http://localhost:5000/update-gender", {
        username: userData.username,
  ...
  return (
    <div className="user-info">
      <div className="profile-container">
        <img 
          src={userData.profileImage ? `/images/${userData.profileImage}` : profileImage} 
          alt="Profile" 
        ...
        <div className="profile-details">
          ...
          <span style={{ marginLeft: '-335px', fontSize: '1.5em', color: '#555'}}>성별 :</span>
  <div 
...
function StampBoard({ userData }) {
  ...
  const i_StampsLow = Array(10).fill(false).map((_, index) => index < imageLowStamps);
  const i_StampsMiddle = Array(10).fill(false).map((_, index) => index < imageMiddleStamps);
  const i_StampsHigh = Array(10).fill(false).map((_, index) => index < imageHighStamps);
  const c_StampsLow = Array(10).fill(false).map((_, index) => index < combineLowStamps);
  const c_StampsMiddle = Array(10).fill(false).map((_, index) => index < combineMiddleStamps);
  const c_StampsHigh = Array(10).fill(false).map((_, index) => index < combineHighStamps);
    ...
    <div>
      ...
      <div className="bar-graph">
        <div className="bar" style={{ width: `${(learnCon / 98) * 100}%` }}>
          <span className="bar-text">{`${Math.floor((learnCon / 98) * 100)}%`}</span>
        </div>
      </div>
      <p>모음 학습하기 ({learnVow} / 60)</p>
      <div className="bar-graph">
        <div className="bar" style={{ width: `${(learnVow / 60) * 100}%` }}>
          <span className="bar-text">{`${Math.floor((learnVow / 60) * 100)}%`}</span>
        </div>
      </div>
      <p>쌍자음 학습하기 ({learnDouCon} / 25)</p>
      <div className="bar-graph">
        <div className="bar" style={{ width: `${(learnDouCon / 25) * 100}%` }}>
          <span className="bar-text">{`${Math.floor((learnDouCon / 25) * 100)}%`}</span>
        </div>
      </div>
      <p>쌍모음 학습하기 ({learnDouVow} / 62)</p>
      <div className="bar-graph">
        <div className="bar" style={{ width: `${(learnDouVow / 62) * 100}%` }}>
          <span className="bar-text">{`${Math.floor((learnDouVow / 62) * 100)}%`}</span>
        </div>
      </div>
    </div>
  );
